...
def micro1():
    ...
    if float(data[neighborhood][0]) > highest:
        highestNeighborhood = data[neighborhood]
    if float(data[neighborhood][0]) < lowest:
        lowestNeighborhood = data[neighborhood]

    return render_template('micro.html', highestNeighborhood=highestNeighborhood, highest=highest, lowestNeighborhood=lowestNeighborhood, lowest=lowest, name=name, map=name+'.svg', fills=fills, data=data, data2=data2, averageData = 260-(averageData*20), numericalAverage=averageData, points=points, finalPoint=float(finalPoint)+5.5)
