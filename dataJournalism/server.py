...
def micro1():
    ...
    for value in data2["Manhattan"]:
        manhattanPoints = manhattanPoints + str(40+((year-2007)*23.3333)) + "," + str(260-(float(value))*20) + " "
        year += 1
        if year==2016:
            finalPoint = str(260-(float(value))*20)

        
    manhattanPoints = manhattanPoints[:-1]

    return render_template('microManhattan.html', data=data, data2=data2, averageData = 260-(averageData*20), numericalAverage=averageData, manhattanPoints=manhattanPoints, finalPoint=float(finalPoint)+5.5)
